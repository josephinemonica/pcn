...
class BatchData(dataflow.ProxyDataFlow):
    ...
    def _aggregate_batch(self, data_holder, use_list=False):
        ...
        if self.input_size is None:
            inputs = [resample_pcd(x[1], self.input_size) if x[1].shape[0] > 3000 else x[1]
        ...
        if self.input_size is None:
            npts = np.stack([x[1].shape[0] if x[1].shape[0] < 3000 else 3000
                             for x in data_holder]).astype(np.int32)
        else:
            npts = np.stack([inputs[0].shape[0]]).astype(np.int32)
